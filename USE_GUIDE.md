...
5. 设置到期日期：
   - 输入天数（如 `30`）= 30天后过期
   - 输入日期（如 `2026-03-01`）= 指定日期过期
   - 留空 = 永不过期
6. 用户自动生成 UUID 并添加到配置
...
═════════════════════════════════════════════
  用户名     已用      配额       使用率   状态  到期
─────────────────────────────────────────────
  default    12.5 GB   无限       -        ●     永久
  张三       45.2 GB   100 GB     45%      ●     25天
  李四       88.0 GB   100 GB     88%      ●     3天   (黄色)
  王五       102.3 GB  100 GB     102%     ○     已过期 (红色)
─────────────────────────────────────────────
```
...
3. 输入新的到期时间：
   - 输入天数：`30` = 从今天算30天后
   - 输入日期：`2026-03-01` = 指定日期
   - 输入 `0` = 取消到期限制（永不过期）

**续期场景**：过期用户设置新到期日期后，会询问是否立即启用。
